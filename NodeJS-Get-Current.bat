...
if "%PROCESSOR_ARCHITECTURE%" == "AMD64" (
  ...
  %CURL% --progress-bar -k https://nodejs.org/download/release/latest/win-x64/node.exe -o "App\node.exe"
) else if "%PROCESSOR_ARCHITECTURE%" == "x86" (
  echo   Get Latest NodeJS Current x86
  %CURL% --progress-bar -k https://nodejs.org/download/release/latest-v17.x/win-x86/node.exe -o "App\node.exe"
) else exit
...
echo   Get Latest NPM
%CURL% --progress-bar -k https://registry.npmjs.org/npm/-/npm-%NpmVers%.tgz -o "tmp\npm-%NpmVers%.tgz"
